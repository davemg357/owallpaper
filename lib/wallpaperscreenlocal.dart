...
class _WallpaperScreenLocalState extends State<WallpaperScreenLocal> {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            TextButton(
                onPressed: () => _setWallpaperWithLoading(WallpaperManagerFlutter.homeScreen),
                child: const Text("Homescreen")),
            TextButton(
                onPressed: () => _setWallpaperWithLoading(WallpaperManagerFlutter.lockScreen),
                child: const Text("Lockscreen")),
            TextButton(
                onPressed: () => _setWallpaperWithLoading(WallpaperManagerFlutter.bothScreens),
  ...
  // Set wallpaper with loading dialog and navigate back
  Future<void> _setWallpaperWithLoading(int location) async {
    ...
    try {
      ...
      final success = await wallpaperManager.setWallpaper(File(path), location);

      Navigator.pop(context); // close loading dialog

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(success ? "Wallpaper set!" : "Failed to set wallpaper"),backgroundColor:success ? Colors.green :Colors.red),
      );

      Navigator.pop(context); // go back after setting wallpaper
    } catch (e) {
      Navigator.pop(context); // close loading dialog
      ScaffoldMessenger.of(context).showSnackBar(
